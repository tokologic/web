<div class="row">
    <div class="col-md-12">
        <form action="{{route('sale.item.store', [$sale_item->id])}}" method="post" id="form-sale-item-add"> {{csrf_field()}}
            ...
            <div class="form-group">
                ...
                <select name="store_item_id" id="store_item_id" class="form-control">
                    @foreach($stallItems as $item)
                        <option value="{{ $item->id }}">{{ $item->product->name }} - {{ rupiah($item->retail_price) }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="amount" class="form-control form-control-sm" name="amount">
                <div class="invalid-feedback"></div>
            </div>
...
<script>
    $('#modal .btn-save').on('click', function (event) {
        ...
        store('form-sale-item-add', 'dataTables-sale-item-list');
    });
